...
# The network consists of four organizations with one
# peer each, and a single node Raft ordering service. Users can also use this
# script to create channels and deploy chaincodes on the channels
...
# The script uses Docker Compose to bring up five CAs, one for each peer organization
...
#######################################################
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    #-f compose/$CONTAINER_CLI/${CONTAINER_CLI}-$COMPOSE_FILE_CA
...
# genesis block is defined using the "FourOrgsApplicationGenesis" profile at the bottom
# of the file. This profile defines an application channel consisting of our four Peer Orgs.
...
# file for creating the network is "compose-test-net.yaml" in the same
...
function networkUp() {
  ...
  #-f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_BASE}

  if [ "${DATABASE}" == "couchdb" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f compose/${COMPOSE_FILE_COUCH}"
    #-f compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_COUCH}
  fi
...
# call the script to create the channel, join the peers of all orgs,
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm $(${CONTAINER_CLI} volume ls -q)
